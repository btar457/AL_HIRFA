...
class _ArtisanChatScreenState extends State<ArtisanChatScreen> {
  ...
  Future<void> _sendMessage() async {
    ...
    setState(() {
      _isSending = true;
      _messages.add({'isUser': true, 'text': text, 'time': _now()});
      _messages.add({
        'isUser': false,
    ...
    try {
      ...
      setState(() {
        ...
        _messages.add({'isUser': false, 'text': _formatAuditResult(result), 'time': _now()});
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _messages.removeLast();
        _messages.add({'isUser': false, 'text': 'تعذّر إتمام التدقيق: ${e.toString().replaceFirst('Exception: ', '')}', 'time': _now()});
      });
    } finally {
